...
MCP_INSTRUCTIONS = """
Ты подключён к Kanban-доске Boardy.

## Структура колонок
- Backlog (position 0) — новые задачи
- In Progress (position 1) — в работе
- Review (position 2) — на проверке
- Done (position 3) — завершённые

## Workflow
1. Обсуждается задача → проверь карточку (search_cards) → создай если нет
2. Задача завершена → close_card + move_card в Done
3. Блокер → add_comment + update_card(status="blocked")
4. Git push/merge → close_card + move_card в Done

## Правила
— Закрыл карточку = переместил в Done (всегда оба действия)
— Взял в работу = переместил в In Progress
— Новые карточки создавай в Backlog
— Для задач AI используй assignee_id="ai-agent"
— Не удаляй карточки, используй archive_card
